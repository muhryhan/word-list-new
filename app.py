...
def error(keyword):
    ...
    if not definitions:
        error_message = f'Could not find, "{keyword}"'
    elif type(definitions[0]) is str:
        error_message = f'Your word, "{keyword}", could not be found. \n \n Here are some suggested words {", ".join(definitions)}?'

    status = request.args.get('status_give', 'new')
    return render_template('error.html', keyword=keyword, error_message=error_message, suggestion=definitions, status=status)
